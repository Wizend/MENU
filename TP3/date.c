...
unsigned short nbJours (enum Mois mois);    // FINIS (pas test)
bool bissextile (unsigned annee);           // FINIS (pas test)
const char* MoisToString (enum Mois mois);  // FINIS (pas test)

Date* creer_Date();                         // FINIS (pas test)
void liberer_Date (Date** d);               // FINIS (pas test)
...
short int comparer_Date (const Date* dl, const Date* d2); // #include <time.h> : utiliser difftime()
...
Date* creer_Date() {
    ...
    Date d = { "Vendredi", MoisToString(1), bissextile(2023) };
}

void liberer_Date (Date** d) { // libération du pointeur pas besoins d'allocation mémoire pr la structure
...
void initialiser_date (Date* d) {
    ...
    printf("\nDate n°%d (jj/mm/aaaa", d[i]);
        if (scanf("%hu %d %u", d->jour, d->mois, d->annee) != 3) {
    ...
    printf("Date : %hu - %d - %u", d->jour, d->mois, d->annee);
    return 0;
}
